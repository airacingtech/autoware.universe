  ...
  for (auto tracker_itr = list_tracker_.begin(); tracker_itr != list_tracker_.end();
    ...
    if (found) {
      ...
      if (update_guard_result.reject) {
        ...
        // second UUID from the rejected alternate depth mode in this cycle. A short, moderate
        // radial disagreement gets a non-resetting grace; hard >5 m mode switches and prolonged
        // disagreement decay normally so an already-wrong track can be reacquired.
        if (update_guard_result.preserve_existence) {
          (*tracker_itr)->coastAfterRejectedMeasurement();
          ++update_guard_grace_count_;
        } else {
  ...
  const rclcpp::Time & time, const types::InputChannel::BirthGuard::UpdateGuard & config) const
{
  ...
  result.preserve_existence = result.reject && elapsed_sec <= config.max_elapsed_sec;
  return result;
}
...
void TrackerProcessor::logBirthGuardStats(const char * event, const uint channel_index) const
{
  RCLCPP_INFO_THROTTLE(
    ...
    static_cast<unsigned long long>(update_guard_grace_count_),
